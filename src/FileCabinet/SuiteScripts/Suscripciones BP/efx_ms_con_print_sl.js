                ...
                var searchDetail = search.create({
                    ...
                    columns: [
                        ...
                        /*{
                             ...
                             formula: "CASE WHEN {custrecord_efx_ms_con_contact} = 'F'              THEN                       CASE WHEN {custrecord_efx_ms_con_ship_cus.isperson} = 'F'                             THEN         {custrecord_efx_ms_con_ship_cus}||' '||{custrecord_efx_ms_con_ship_cus.companyname}                WHEN {custrecord_efx_ms_con_ship_cus.isperson} = 'T'                     THEN {custrecord_efx_ms_con_ship_cus}||'  '|| {custrecord_efx_ms_con_ship_cus.firstname}||' '||{custrecord_efx_ms_con_ship_cus.lastname}             END ||'          '||'<br />'||'                '||{custrecord_efx_ms_con_ship_cus.billaddress1}||'                     '||'No. ext.: '||' '||' _ '||'       '||'No. int.: '||' '||{custrecord_efx_ms_con_ship_cus.billaddress3}||'         '|| '<br />' ||'        '||{custrecord_efx_ms_con_des}||'       '|| '<br />' ||'       '||'Col.: '||' '||{custrecord_efx_ms_con_ship_cus.billaddress2}||'        '|| '<br />' ||'       '||'CP: '||' '|| {custrecord_efx_ms_con_ship_cus.billzipcode} ||' '||{custrecord_efx_ms_con_ship_cus.billcity}||'        '||CASE WHEN INSTR({custrecord_efx_ms_con_ship_cus.billstate} , 'Ciudad de México') > 0           THEN 'CDMX'           WHEN INSTR({custrecord_efx_ms_con_ship_cus.billstate} , 'México (Estado de)') > 0           THEN 'Estado de México'           ELSE {custrecord_efx_ms_con_ship_cus.billstate}            END||'       '|| '<br />' ||'       '||{custrecord_efx_ms_con_ship_cus.billcountry}                      WHEN {custrecord_efx_ms_con_contact} = 'T'     THEN        {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_atention}||'       '||'<br />'||'                '|| {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_street}||'                     '||'No. ext.: '||' '|| {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_number}||'                       '||'No. int.: '||' '|| {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_int_number}||'                     '|| '<br />' ||'        '||{custrecord_efx_ms_con_des}||'       '|| '<br />' ||'       '||'Col.: '||' '|| {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_col} ||'        '|| '<br />' ||'       '||'CP: '||' '|| {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_zip_code} ||' '||{custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_city}||'        '||CASE WHEN INSTR({custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_state} , 'Mexico City') > 0           THEN 'CDMX'           WHEN INSTR({custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_state} , 'México (Estado de)') > 0           THEN 'Estado de México'           ELSE {custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_state}            END||'       '|| '<br />' ||'       '||{custrecord_efx_ms_con_ship_cont.custrecord_efx_ms_ca_country}          END"
                         }*/
                        {
                            name: 'formulatext_cust_address',
                            formula:
